...
int sprintf(char * __restrict__, const char * __restrict__, ...) __attribute__((__format__ (__printf__, 2, 3))) __attribute__((__availability__(swift, unavailable, message="Use snprintf instead.")));
...
int vsprintf(char * __restrict__, const char * __restrict__, va_list) __attribute__((__format__ (__printf__, 2, 0))) __attribute__((__availability__(swift, unavailable, message="Use vsnprintf instead.")));
...
rb_vm_search_method_slowpath(struct rb_call_data *cd, VALUE klass)
{
    ...
    __builtin___memcpy_chk ((cc), (&buf), sizeof(struct rb_call_cache)*(size_t)(1), __builtin_object_size ((cc), 0));
    ((void)0);
}
static inline _Bool
...
CALLER_SETUP_ARG(struct rb_control_frame_struct *__restrict__ cfp,
                 ...
                 const struct rb_call_info *__restrict__ ci)
{
